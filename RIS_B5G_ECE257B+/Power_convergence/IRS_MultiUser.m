% Optimize IRS phase shifts v for multi-user scenario (Problem P4)
...
%     R = cell(Uk,Uk);
    ...
    cvx_begin quiet
        ...
        subject to
            for i=1:Uk
                real(trace(R(:,:,k,j)*V))+square_abs(b(i,i))>=gamma*real(sum(RV(i,[1:i-1 i+1:Uk]))+gamma*(b(i,[1:i-1 i+1:Uk])*b(i,[1:i-1 i+1:Uk])'+1) + alpha1(i));
                alpha1(i) >= 0;
            end
            diag(V) == 1;
    ...
    for l = 1 : L
        ...
        for i=1:Uk
            s = s + real(v'*R(:,:,k,j)*v+b(i,i)'*b(i,i)-gamma*sum(RVp(i,[1:i-1 i+1:Uk]))-gamma*(b(i,[1:i-1 i+1:Uk])*b(i,[1:i-1 i+1:Uk])'+1));
        end
        if real(s)> max_F
            max_v = v;
            max_F = real(s);
